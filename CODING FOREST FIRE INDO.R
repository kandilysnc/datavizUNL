...
library('utils')
library('magrittr')
#import the data in CSV spreadsheet
INDfires <- read.csv ('~/Data/Archive_fires_for_IndonesiaedtWed.csv')
#import the points, the centralized longitude and latitude in shapefile
points <- read_sf('~/Downloads/utami-2/centroids.shp')
#import the maps, im trying to use the edited version
INDOmap <- read_sf('~/Downloads/utami-2/simplified.shp')
...
#calling out the bubble map using ggplot etc
